...
newTechnologies: [Zod 4, Sharp, architecture plugin data-driven, packages publiables Nx]
...
- **`dependsOn`** — ordre de *résolution* des choix (amont avant aval)
- **`order`** — z-index de *peinture* (distinct de `dependsOn`)
...
- **Zod 4** : une seule définition doit servir runtime, TypeScript *et* JSON Schema éditeur — sans faire fuiter Zod hors du SDK.
...
- Documenter le *pourquoi* (architecture.md, roadmap) en anglais pour l'open source, et garder `.devbook/` pour l'apprentissage personnel / portfolio.
