...
| `variants`    | object     | no       | Batch matrix; see below.                          |
...
Declares a cartesian product of layer options for `klairox batch`. Option lists come from
the layers themselves — axes only name which layers to expand.

| Field      | Type          | Default       | Notes                                                                 |
| ---------- | ------------- | ------------- | --------------------------------------------------------------------- |
| `axes`     | id[]          | —             | At least one layer id. Crossed in declaration order.                  |
| `include`  | object        | `{}`          | Fixed `layer → option` choices applied to every variant. Not on axes. |
| `exclude`  | object[]      | `[]`          | Combinations to drop before rendering.                                |
| `name`     | string        | derived       | Output base name. Placeholders: `{plugin}`, `{<layerId>}`.            |
| `strategy` | `"cartesian"` | `"cartesian"` | Only strategy today.                                                  |

When `name` is omitted, the engine builds `{plugin}-{axis1}-{axis2}-…`. Combinations that
break a constraint are skipped with a warning rather than failing the whole batch.
...
CLI overrides: `--axis` replaces `axes`, `--select` merges into `include`, `--name` sets the
template. Use `--dry-run` to list the matrix without writing files.
...
upstream choice, a `variants` matrix for batch generation, and multi-format export with a
thumbnail. Realistic coats are stamped onto a shared silhouette with
`tools/stamp-coat-on-morph.mjs`. Placeholder silhouettes still come from
`tools/generate-example-assets.mjs` if you need to reset schematic art.
