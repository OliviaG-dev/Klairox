/**
 * Install the hand-cut Standard-OC bay plate (RGBA) into Klairox.
 * Does not touch foals or Stalloria.
 *
 *   node tools/install-standard-bay-cutout.mjs
 *   node tools/install-standard-bay-cutout.mjs <cutout.png>
 */
...
const DEFAULT_CUTOUT = path.join(
  ROOT,
  'docs/images/horse-source/coat-bay-photoreal-cutout.png',
);

const OUT_SRC = path.join(
  ROOT,
  'docs/images/horse-source/coat-bay-photoreal-src.png',
);
const OUT_PLUGIN = path.join(ROOT, 'plugins/horse/layers/coat/bay.png');
...
console.log(
  `installed Standard-OC bay cutout from ${path.relative(ROOT, cutoutPath)}`,
);
console.log(
